...
/*   Updated: 2019/06/21 13:37:21 by amazhara         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	info(void)
{
	...
	mvwprintw(g_wis.win,5,215,"Cycles : %zd", g_cycles);
	mvwprintw(g_wis.win,7,215,"Process : %zd", g_carriers->len);
	mvwprintw(g_wis.win,9,215,"Cycle to die : %d", g_cycles_die);
	mvwprintw(g_wis.win,11,215,"CYCLE_DELTA : %zd", CYCLE_DELTA);
	mvwprintw(g_wis.win,13,215,"NBR_LIVE : %zd", NBR_LIVE);
	mvwprintw(g_wis.win,15,215,"MAX_CHECKS : %zd", g_checks_count);
	show_champs();
	wattroff(g_wis.win, A_BOLD);
}
