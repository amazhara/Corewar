...
void	add(t_op_inf *inf)
{
	...
	carr->regs[reg] = carr->regs[byte_get(inf->pos_read, 1)] + carr->regs[byte_get(inf->pos_read + 1, 1)];
	if (g_vl & (uint)s_operations)
		ft_printf("P %4d | add r%d r%d r%d\n", inf->carr->id, byte_get(inf->pos_read, 1), byte_get(inf->pos_read + 1, 1), reg);
	carr->flag = 0;
	if (!carr->regs[reg])
		carr->flag = 1;
}
...
//	ft_printf("size %d %d\n", args_size[0], args_size[1]);
//	ft_printf("num %d %d\n", args_num[0], args_num[1]);
	args[2] = byte_get(inf->pos_read + args_size[0] + args_size[1], byte_option_arg(inf->arg_type[2]));
