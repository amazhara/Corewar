...
{
	int 			id;
	int				champ_id; // Стартовый чемпион, на которого ставится каретка
	unsigned char	flag; // Флаг, который могут изменять некоторые операции. Изначально его значение равно false
	unsigned char	code_curr; // Код операции, на которой стоит каретка, До начала битвы значение этой переменной не установлено
	int				cycles_id; // Цикл, в котором в последний раз была выполнена операция live
	int				cycles_to_exec; // Количество циклов, оставшиеся до исполнения операции, на которой стоит каретка
	unsigned char	*pos; // Текущая позиция каретки
	int				skip; // Количество байт, которые нужно будет «перешагнуть», чтобы оказаться на следующей операции
	int				regs[REG_NUMBER + 1]; // Регистры, количество которых задано в константе REG_NUMBER
	bool			dead;
};
...
static t_op		g_op_tab[] =
{
	...
	{"and", 3, {T_REG | T_DIR | T_IND, T_REG | T_IND | T_DIR, T_REG}, 6, 6, "et (and  r1, r2, r3   r1&r2 -> r3", 1, 0, &byte_and},
	{"or", 3, {T_REG | T_IND | T_DIR, T_REG | T_IND | T_DIR, T_REG}, 7, 6, "ou  (or   r1, r2, r3   r1 | r2 -> r3", 1, 0, &byte_or},
	{"xor", 3, {T_REG | T_IND | T_DIR, T_REG | T_IND | T_DIR, T_REG}, 8, 6, "ou (xor  r1, r2, r3   r1^r2 -> r3", 1, 0, &byte_xor},
